...
#define OPCODE_LOADS(code, register_name) MONADIC_OPCODE(code) { \
  cpu.register_name = mode.Read(cpu); \
  cpu.UpdateFlagsFor(cpu.register_name); \
}
#define OPCODE_STORES(code, register_name) MONADIC_OPCODE(code) { \
  mode.Write(cpu, cpu.register_name); \
}
#define OPCODE_TRANSFERS(code, src, dest) NILADIC_OPCODE(code) { \
  cpu.dest = cpu.src; \
