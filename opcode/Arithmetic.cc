...
NILADIC_OPCODE(INX) {
  cpu.UpdateFlagsFor(++cpu.x);
}
NILADIC_OPCODE(INY) {
  cpu.UpdateFlagsFor(++cpu.y);
}
NILADIC_OPCODE(DEX) {
  cpu.UpdateFlagsFor(--cpu.x);
}
...
#define OPCODE_UPDATES_FLAGS_AND_MAKES_ACCUMULATOR(code, new_ac) MONADIC_OPCODE(code) { \
  cpu.ac = (new_ac); \
  cpu.UpdateFlagsFor(cpu.ac); \
}
...
#define OPCODE_MODIFIES_VALUE_AND_SETS_CARRY(code, carry_value, result_value) MONADIC_OPCODE(code) { \
  mode.Modify(cpu, [&](byte value) { \
...
#define OPCODE_COMPARES_THIS_WITH_MEMORY(code, register_name) MONADIC_OPCODE(code) { \
  const auto operand(mode.Read(cpu)); \
