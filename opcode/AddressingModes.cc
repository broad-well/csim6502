...
#define DEFINE_MEMORY_MODE(name, operation)                        \
  namespace _##name##_ {                                           \
    byte Read(CPU &cpu) { return cpu.memory->Read((operation)); }  \
...
DEFINE_MEMORY_MODE(XInd, cpu.memory->ReadWord(bit::LowByte(cpu.NextCodeByte() +
                                                           cpu.x)))
...
AddressingMode Immed{ImRead, ImWrite, ImModify};
...
AddressingMode Accum{AcRead, AcWrite, AcModify};
