...
#define OPCODE_BRANCHES_IF(code, condition)                           \
  NILADIC_OPCODE(code) {                                              \
    if (condition)                                                    \
      cpu.JumpRelative(static_cast<signed_byte>(cpu.NextCodeByte())); \
  }
#define OPCODE_BRANCHES_IF_FLAG_SET(code, flag) \
  OPCODE_BRANCHES_IF(code, cpu.status.flag)
#define OPCODE_BRANCHES_IF_FLAG_CLEAR(code, flag) \
  OPCODE_BRANCHES_IF(code, !cpu.status.flag)
...
OPCODE_SET {
  ...
  NILADIC_OPCODE(JMP_Abs) { cpu.pc = cpu.NextOperandWord(); }
